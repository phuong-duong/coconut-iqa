...
| `5_wholetree_decline` | Suy tàn toàn cây (sàng lọc bệnh rễ/rụng chồi) | **Toàn cây/dáng cây** từ xa | Bud Root Dropping |
...
| LF5 `correct_wholetree` | Model suy-tàn-toàn-cây vs lớp GT | `5_wholetree` | chẩn đúng → 1; sai → 0 | ảnh không có GT lớp này | **Cao** |
...
2. **LF1–LF5 (correctness)**: tích hợp 5 mô hình hạ nguồn (độ-chín / bệnh-lá / bệnh-thân / bệnh-đọt / suy-tàn-toàn-cây) vào `notebooks/label_correctness.ipynb`. Đã có sẵn phần đọc GT (kể cả nhãn mức chín `dry/green/tender`) + logic; chỉ cần bổ sung hàm suy luận của mô hình.
...
- Đã có sẵn 5 model hạ nguồn (độ-chín, bệnh-lá, bệnh-thân, bệnh-đọt, suy-tàn-toàn-cây) chưa? Nếu chưa, cần huấn luyện/tải trước khi LF1–5 chạy được. Model độ chín có thể train ngay trên nhãn `dry/green/tender` của bộ Roboflow.
...
- `5_wholetree_decline` là tín hiệu **kém đặc hiệu** (dáng cây suy tàn do nhiều nguyên nhân) → phù hợp sàng lọc bước đầu; ghi rõ giới hạn này khi báo cáo.
